...
public class BagCucumberStepDefinitions {
	 ...
	 private final Logger log = LoggerFactory.getLogger(BagCucumberStepDefinitions.class);
		...
		/*
		 * IntStream.range(0, quantity) .peek(n ->
		 * log.info("Putting a {} in the bag, number {}", something, quantity))
		 * .map(ignore -> bagHttpClient.put(something)) .forEach(statusCode ->
		 * assertThat(statusCode).isEqualTo(HttpStatus.CREATED.value()));
		 */
	    	log.info("Putting a {} in the bag, number {}", something, quantity);
	    	int statusCode = 0;
	    	for(int i =1;i<=quantity;i++) {
	    		statusCode = bagHttpClient.put(something);
	    	}
	    	assertThat(statusCode).isEqualTo(HttpStatus.CREATED.value());
	    }
